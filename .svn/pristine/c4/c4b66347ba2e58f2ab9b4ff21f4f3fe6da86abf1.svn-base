...
public class BetSlipSteps extends InRunningBaseStep {
    ...
    @Aliases(values = { "Then <<Selection>> of <<MarketTitle>> should still be displayed in the betslip" })
    ...
    public void removeSelection(@Named("<Selection>") final String selection) {
        deleteTheSelection(selection);
    }

    @When("player remove selection <<Selection2>> from betslip")
    public void removeSelection2(@Named("<Selection2>") final String selection) {
        deleteTheSelection(selection);
    }
    ...
    public void verifyMaxSelectionsErrorMessage() throws InterruptedException {
    ...
    @Then("a error message '$errorMsg' to be displayed")
    ...
    public void verifyGeneralQBMErrorMessage(final String errorMsg) throws InterruptedException {
        betslipHelperMethods.verifyBetSlipGeneralQBMErrorMessage(errorMsg);
    }
    ...
    public void verifyNotLoggedInMessage() {
        ...
        final String actualMessageText = betSlipModeComponent.getGeneralQBMErrorMessage();
    ...
    @Then("the message border should display below color $selectionTable")
    public void verifyMsgBorderColor(final ExamplesTable selectionTable) {
        final Utils utils = new Utils();
        StringBuilder failSelections = new StringBuilder();
        for (int rowNo = 0; rowNo < selectionTable.getRowCount(); rowNo++) {
            final Map<String, String> selectionData = utils.getParamsFromExamplesTable(selectionTable, rowNo);
            final String selection = selectionData.get(SELECTION);
            final String actualBorderColor = betslipHelperMethods.getMsgBorderColor(selection);
            final String expectedBorderColor = selectionData.get("Color");
            if (!expectedBorderColor.equals(actualBorderColor)) {
                failSelections.append("\n" + selection + ",");
            }
        }
        Assert.assertTrue(StringUtils.isEmpty(failSelections.toString()),
                "Expected border colors are displayed for the selection",
                String.format("Expected border colors are not displayed for below selection :%s", failSelections));
    ...
    public void verifySelectionDisabled(@Named("Selection2") final String selection) {
        new WaitUtils().waitForSelectionMessage(selection);
        final BetSlip betSlip = getLiveBettingPage().getBetSlip();
        final BetSlipSelection betSlipSelection = betSlip.getSelectionByDescription(selection);
        final boolean isSuspended = betSlipSelection.isSelectionSuspended();
        Assert.assertTrue(isSuspended, String.format("%s : selection is disabled in the betslip", selection),
                String.format("%s : selection is not disabled in the betslip", selection));
    }

    @Then("relevant below selections should be disabled in the betslip $selectionsTable")
    public void verifyDisabledSelection(final ExamplesTable selectionTable) {
        final Utils utils = new Utils();
        for (int rowNo = 0; rowNo < selectionTable.getRowCount(); rowNo++) {
            Map<String, String> selectionData = utils.getParamsFromExampleTableRow(selectionTable, rowNo);
            verifySelectionDisabled(selectionData.get(SELECTION));
        }
    }

    @Then("Free bet balance should be displayed in the Betslip mode")
    public void verifyFreeBetBalanceInBetslipMode() {
        final BetSlip betSlip = getLiveBettingPage().getBetSlip();
        boolean freeBetDisplayed = Boolean.FALSE;
        if (betSlip.isFreeBetTextDisplayed() && betSlip.isFreeBetBalanceDisplayed()) {
            freeBetDisplayed = Boolean.TRUE;
        }
        Assert.assertTrue(freeBetDisplayed, "Free bet balance is displayed in Bet slip mode",
                "Free bet balance is not displayed in Bet slip mode");
    }

    @Then("Free bet balance should be displayed in the QBM mode")
    public void verifyFreeBetBalanceInQBMMode() {
        final BetSlipModeComponent betSlipModeComponent = getLiveBettingPage().getBetSlipModeComponent();
        boolean freeBetDisplayed = Boolean.FALSE;
        if (betSlipModeComponent.isFreeBetTextDisplayed() && betSlipModeComponent.isFreeBetBalanceDisplayed()) {
            freeBetDisplayed = Boolean.TRUE;
        }
        Assert.assertTrue(freeBetDisplayed, "Free bet balance is displayed in Quick bet mode",
                "Free bet balance is not displayed in Quick bet mode");
    }

    @Then("elipses should be displayed for the selection <Outcome Description>")
    public void verifyElipsesForSelectionDesc(@Named("<Outcome Description>") final String outcomeDesc) {
        final BetSlip betSlip = getLiveBettingPage().getBetSlip();
        final BetSlipSelection betSlipSelection = betSlip.getSelectionByDescription(outcomeDesc);
        final boolean elipsesDisplayed = betSlipSelection.isElipsesApplied();
        Assert.assertTrue(elipsesDisplayed, "Elipses is displayed for the selection",
                "Elipses is not displayed for the selection");
    }
